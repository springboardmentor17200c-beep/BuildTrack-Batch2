...
async def milestones(current_user=Depends(get_current_user), db=Depends(get_database)):
    return await list_collection(db, "project_milestones")
...
async def delete_milestone(record_id: str, current_user=Depends(require_admin), db=Depends(get_database)):
    return await delete_record(db, "project_milestones", record_id)


@router.get("/tasks")
async def tasks(current_user=Depends(get_current_user), db=Depends(get_database)):
    return await list_collection(db, "project_tasks")
...
async def delete_task(record_id: str, current_user=Depends(require_admin), db=Depends(get_database)):
    return await delete_record(db, "project_tasks", record_id)


@router.get("/documents")
async def documents(current_user=Depends(get_current_user), db=Depends(get_database)):
    return await list_collection(db, "project_documents")
...
async def delete_document(record_id: str, current_user=Depends(require_admin), db=Depends(get_database)):
    return await delete_record(db, "project_documents", record_id)
