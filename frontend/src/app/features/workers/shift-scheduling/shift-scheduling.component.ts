    ...
    <div class="page">
      ...
      <div class="panel">
        ...
        <div class="shift-grid">
          ...
          <p class="empty" *ngIf="shifts.length === 0">Add workers in Worker Management to prepare shift schedules.</p>
        </div>
      </div>
    </div>
  `,
...
export class ShiftSchedulingComponent {
  ...
  get shifts() {
    return this.data.workers.slice(0, 12).map((worker) => ({
      worker: worker.name,
      role: worker.skillType || worker.role,
      window: worker.shiftWindow || 'General - 9 AM - 6 PM',
